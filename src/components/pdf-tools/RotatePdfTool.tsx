...
import { ArrowDown, ArrowUp, Download, FileText, RotateCw, Loader2, ShieldCheck, Trash2, Upload } from "lucide-react";
import { rotatePdfPages } from "@/lib/pdf/operations/advanced";
      ...
      <header className={styles.header}>
        <div className={styles.brandGroup}>
          ...
          <div>
            ...
            <h1>Rotate PDF</h1>
            <p className={styles.headerDescription}>Putar halaman terpilih ke orientasi yang benar.</p>
          </div>
        </div>
      ...
      <section className={styles.toolWorkbench} aria-label="PDF tools workspace">
        ...
        <aside className={styles.actionPanel} aria-label="Tool options">
          ...
          <div className={styles.optionStack}>
            ...
            <SelectField label="Degrees" value={String(rotateDegrees)} onChange={(value) => setRotateDegrees(Number(value) as 90 | 180 | 270)} options={[["90", "90 degrees"], ["180", "180 degrees"], ["270", "270 degrees"]]} />
          </div>
          ...
          <div className={styles.actionButtons}>
            <button className={styles.primaryButton} type="button" onClick={() => void tool.handleProcess(() => { const document = requireActiveDocument(tool.activeDocument); return rotatePdfPages(document.bytes, parsePages(pageRange, document.pageCount), rotateDegrees); })} disabled={!(canProcess)}>
