...
import { ArrowDown, ArrowUp, Download, FileText, Stamp, Loader2, ShieldCheck, Trash2, Upload } from "lucide-react";
import { addWatermark } from "@/lib/pdf/operations/advanced";
      ...
      <header className={styles.header}>
        <div className={styles.brandGroup}>
          ...
          <div>
            ...
            <h1>Watermark PDF</h1>
            <p className={styles.headerDescription}>Tambahkan watermark teks ke halaman PDF.</p>
          </div>
        </div>
      ...
      <section className={styles.toolWorkbench} aria-label="PDF tools workspace">
        ...
        <aside className={styles.actionPanel} aria-label="Tool options">
          ...
          <div className={styles.optionStack}>
            ...
            <NumberField label="Opacity" value={watermarkOpacity} onChange={setWatermarkOpacity} min={0.05} max={0.8} step={0.05} />
          </div>
          ...
          <div className={styles.actionButtons}>
            <button className={styles.primaryButton} type="button" onClick={() => void tool.handleProcess(() => addWatermark(requireActiveDocument(tool.activeDocument).bytes, watermarkText, { opacity: watermarkOpacity, fontSize: 42 }))} disabled={!(canProcess)}>
