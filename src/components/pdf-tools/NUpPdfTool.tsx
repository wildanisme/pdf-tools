...
import { nUpPdfPages } from "@/lib/pdf/operations/advanced";
...
export function NUpPdfTool() {
  ...
  const [nUp, setNUp] = useState<2 | 4>(2);
      ...
      <header className={styles.header}>
        <div className={styles.brandGroup}>
          ...
          <div>
            ...
            <h1>N-up PDF</h1>
            <p className={styles.headerDescription}>Susun 2 atau 4 halaman ke satu halaman baru.</p>
          </div>
        </div>
      ...
      <section className={styles.toolWorkbench} aria-label="PDF tools workspace">
        ...
        <aside className={styles.actionPanel} aria-label="Tool options">
          ...
          <div className={styles.optionStack}>
            ...
            <SelectField label="N-up" value={String(nUp)} onChange={(value) => setNUp(Number(value) as 2 | 4)} options={[["2", "2-up"], ["4", "4-up"]]} />
          </div>
          ...
          <div className={styles.actionButtons}>
            <button className={styles.primaryButton} type="button" onClick={() => void tool.handleProcess(() => nUpPdfPages(requireActiveDocument(tool.activeDocument).bytes, nUp))} disabled={!(canProcess)}>
