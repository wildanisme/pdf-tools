...
import { ArrowDown, ArrowUp, Download, FileText, Loader2, ShieldCheck, Trash2, Upload } from "lucide-react";
import { fillAndFlattenForms } from "@/lib/pdf/operations/advanced";
import { TextField, TextareaField, parseKeyValueLines, requireActiveDocument } from "./shared";
      ...
      <header className={styles.header}>
        <div className={styles.brandGroup}>
          ...
          <div>
            ...
            <h1>Fill Forms</h1>
            <p className={styles.headerDescription}>Isi field form teks dan flatten hasilnya.</p>
          </div>
        </div>
      ...
      <section className={styles.toolWorkbench} aria-label="PDF tools workspace">
        ...
        <aside className={styles.actionPanel} aria-label="Tool options">
          ...
          <div className={styles.optionStack}>
            ...
            <TextareaField label="Text fields" value={formValues} onChange={setFormValues} placeholder={"customer_name=Wildan\ninvoice_id=INV-001"} help="Satu field per baris. Field non-teks akan dilewati." />
          </div>
          ...
          <div className={styles.actionButtons}>
            <button className={styles.primaryButton} type="button" onClick={() => void tool.handleProcess(() => fillAndFlattenForms(requireActiveDocument(tool.activeDocument).bytes, parseKeyValueLines(formValues)))} disabled={!(canProcess)}>
