...
import { ArrowDown, ArrowUp, Download, FileText, PanelBottom, Loader2, ShieldCheck, Trash2, Upload } from "lucide-react";
import { resizePdfPages, type PageSizePreset } from "@/lib/pdf/operations/advanced";
import { SelectField, TextField, requireActiveDocument } from "./shared";
...
export function ResizePdfTool() {
  ...
  const [resizePreset, setResizePreset] = useState<Exclude<PageSizePreset, "original">>("a4");
      ...
      <header className={styles.header}>
        <div className={styles.brandGroup}>
          ...
          <div>
            ...
            <h1>Resize PDF</h1>
            <p className={styles.headerDescription}>Ubah ukuran halaman ke A4, Letter, atau Square.</p>
          </div>
        </div>
      ...
      <section className={styles.toolWorkbench} aria-label="PDF tools workspace">
        ...
        <aside className={styles.actionPanel} aria-label="Tool options">
          ...
          <div className={styles.optionStack}>
            ...
            <SelectField label="Target size" value={resizePreset} onChange={(value) => setResizePreset(value as Exclude<PageSizePreset, "original">)} options={[["a4", "A4"], ["letter", "Letter"], ["square", "Square"]]} />
          </div>
          ...
          <div className={styles.actionButtons}>
            <button className={styles.primaryButton} type="button" onClick={() => void tool.handleProcess(() => resizePdfPages(requireActiveDocument(tool.activeDocument).bytes, resizePreset))} disabled={!(canProcess)}>
